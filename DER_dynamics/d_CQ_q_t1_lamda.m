function m = d_CQ_q_t1_lamda(t,q,Lam,LM_S,NUMGEN,EleInf_S)
%UNTITLED3 此处显示有关此函数的摘要
%   此处显示详细说明

NE_S=size(LM_S,1);%NE_S是约束方程的个数%

m=zeros(NUMGEN,NUMGEN);
for i=1:NE_S
    lm=LM_S(i,:);
    m(lm,lm)=[ 2*Lam(i)*eye(3) -2*Lam(i)*eye(3);
              -2*Lam(i)*eye(3)  2*Lam(i)*eye(3) ];
end
